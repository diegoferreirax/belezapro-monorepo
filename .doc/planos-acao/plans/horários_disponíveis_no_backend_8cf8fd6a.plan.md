---
name: Horários disponíveis no backend
overview: Mover o cálculo de `getAvailableTimes` do Angular para um serviço Java reutilizável, expor um endpoint HTTP (primeiro no fluxo público de agendamento) e simplificar o `BookingFormComponent` para consumir a lista já calculada no servidor, com testes e decisão explícita de timezone/Instant no backend.
todos:
  - id: java-availability-service
    content: Implementar serviço Java de disponibilidade (Clock + ZoneId), reutilizando ScheduleService.getConfigForDate e AppointmentService.getActiveByDate, com excludeAppointmentId.
    status: completed
  - id: java-public-endpoint
    content: Expor GET /api/v1/public/professionals/{id}/available-times com query params date, durationMinutes, excludeAppointmentId opcional.
    status: completed
  - id: java-unit-tests
    content: Adicionar testes unitários espelhando os casos do schedule-calculator.service.spec.ts (Clock fixo).
    status: completed
  - id: frontend-consume-api
    content: Adicionar getAvailableTimes em public-booking.service.ts e refatorar booking-form para signal/HTTP com cancelamento de requests obsoletas.
    status: completed
  - id: cleanup-frontend-calculator
    content: Remover ScheduleCalculatorService e spec após migração, ou delegar temporariamente ao backend.
    status: completed
  - id: optional-server-validate
    content: (Fase 2) Validar horário no create/update de agendamento para evitar double-booking sob concorrência.
    status: completed
isProject: false
---
